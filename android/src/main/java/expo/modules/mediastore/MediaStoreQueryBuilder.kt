...
import expo.modules.mediastore.models.PaginationOptionsRecord
...
class MediaStoreQueryBuilder {
  ...
  fun buildGenreProjection(): Array<String> {
    return arrayOf(
  ...
  fun buildPlaylistProjection(): Array<String> {
    return arrayOf(
  ...
  fun buildFilter(filter: FilterOptionsRecord?): String? {
    ...
    filter.mimeTypes?.let { types ->
      if (types.isNotEmpty()) {
        val quoted = types.joinToString(", ") { "'${escapeSql(it)}'" }
    ...
    filter.folder?.let {
      conditions.add("${MediaStore.MediaColumns.RELATIVE_PATH} LIKE '${escapeSql(escapeLike(it))}%'")
    }

    filter.album?.let {
      conditions.add("${MediaStore.Audio.Media.ALBUM} = '${escapeSql(it)}'")
    }

    filter.artist?.let {
      conditions.add("${MediaStore.Audio.Media.ARTIST} = '${escapeSql(it)}'")
    }
    ...
    filter.minResolution?.let {
      conditions.add("(${MediaStore.Video.Media.WIDTH} * ${MediaStore.Video.Media.HEIGHT}) >= $it")
    }
    filter.maxResolution?.let {
      conditions.add("(${MediaStore.Video.Media.WIDTH} * ${MediaStore.Video.Media.HEIGHT}) <= $it")
    }
  ...
  fun buildAlbumFilter(filter: FilterOptionsRecord?): String? {
    ...
    filter.artist?.let {
      conditions.add("${MediaStore.Audio.Albums.ARTIST} = '${escapeSql(it)}'")
    }
  ...
  fun buildDocumentFilter(filter: FilterOptionsRecord?): String? {
    val conditions = mutableListOf<String>()
    val docMimeTypes = MimeUtils.getDocumentMimeTypes()
