...
export default function SearchScreen() {
  ...
  const handleSearch = useCallback(async (text: string) => {
    ...
    setLoading(true);
    try {
      const res = await search({ query: text, types: ["audio", "video", "image", "document"], pagination: { limit: 50 } });
      setResults(res);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View style={styles.container}>
      <TextInput
        ...
        autoCorrect={false}
      />
      {loading && <View style={styles.center}><Text>Searching...</Text></View>}
      {results && !loading && <Text style={styles.count}>{results.totalCount} results</Text>}
      <FlatList
        data={items}
        ...
        ListEmptyComponent={
          query.length >= 2 && !loading
            ? <View style={styles.center}><Text style={{ color: "#888" }}>No results</Text></View>
            : null
        }
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  input: { margin: 16, padding: 12, borderRadius: 10, backgroundColor: "#f0f0f0", fontSize: 16 },
