...
- **Cross-platform** — Android (full implementation), iOS (stub — returns empty results, ready for Photos/Music integration)
...
| Feature | **expo-mediastore** | expo-file-system | react-native-fs |
|---------|:-------------------:|:----------------:|:----------------:|
| Audio (music) | ✅ | ❌ | ⚠️ |
| Video | ✅ | ❌ | ⚠️ |
| Images | ✅ | ❌ | ⚠️ |
...
```typescript
import { useMediaChangeEvent } from "expo-mediastore";
...
console.log(`Video: ${result.videos.length}`);
```
...
Invalidation
 │
 ▼
Refresh from MediaStore
 │
 ▼
Observer (auto-invalidates on changes)
```
...
```typescript
import { getAudio, getAlbums, getArtists, getAlbumArtwork } from "expo-mediastore";
...
```typescript
import { getImages, getImageThumbnail } from "expo-mediastore";
...
```typescript
import { getDocuments, getFolders } from "expo-mediastore";
...
```typescript
import { search, SearchResult } from "expo-mediastore";
...
```typescript
import { getAudio } from "expo-mediastore";
import { useState, useCallback } from "react";
...
```typescript
import { getVideos } from "expo-mediastore";
...
```typescript
import { getAlbums, getAudio, getAlbumArtwork } from "expo-mediastore";
...
```typescript
import { getPlaylists, getAudio } from "expo-mediastore";
...
```typescript
import { getAudio, useMediaChangeEvent } from "expo-mediastore";
...
function useReactiveAudio() {
  ...
  useEffect(() => { refresh(); }, []);
  useMediaChangeEvent(refresh); // re-fetch on any MediaStore change
...
**A:** No. It requires native module support (Expo Dev Build or bare React Native).
...
1.1
  ...
  ☐ Reactive subscriptions (Rx-like)
...
- **Coding style**: Kotlin follows Android conventions; TypeScript follows Prettier + ESLint
- **PR process**: Feature branches → PR → review → squash merge to `main`
- **Branch naming**: `feature/description`, `fix/description`, `chore/description`
- **Commit format**: `[type]: description` (e.g., `feat: add thumbnail support`)
